...
export const HospitalCapabilityPanel: React.FC<HospitalCapabilityPanelProps> = ({
  ...
  return (
    <div className={clsx('border border-[#E2E8F0] rounded-2xl bg-white p-5 shadow-sm select-none', className)}>
      <div className="flex flex-wrap items-center justify-between gap-2 pb-4 border-b border-[#E2E8F0]">
        <div>
          <span className="text-xs font-mono uppercase tracking-wider text-[#475569] font-bold">
            Hospital Telemetry
          </span>
          <h3 className="text-lg font-black text-[#0F172A] tracking-tight">{hospital.name}</h3>
        </div>
      ...
      {editable ? (
        ...
        <div className="space-y-4 pt-4">
          <div className="text-xs font-mono uppercase tracking-wider text-[#149B9E] font-bold">
          ...
          <div className="pt-2 flex items-center justify-between">
            {savedNote ? (
              <span className="text-sm font-mono text-[#0D9488] font-bold">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 pt-4 font-mono text-xs">
          <div className="bg-[#F8FAFC] p-3 rounded-xl border border-[#E2E8F0]">
            <div className="text-[#475569] text-[10px] font-bold uppercase">Trauma Team</div>
            <div className={clsx('text-sm font-black mt-0.5', hospital.trauma_team_on_shift ? 'text-[#0D9488]' : 'text-[#475569]')}>
              {hospital.trauma_team_on_shift ? 'ON SHIFT' : 'OFF'}
            </div>
          </div>

          <div className="bg-[#F8FAFC] p-3 rounded-xl border border-[#E2E8F0]">
            <div className="text-[#475569] text-[10px] font-bold uppercase">ICU Beds Free</div>
          ...
          <div className="bg-[#F8FAFC] p-3 rounded-xl border border-[#E2E8F0]">
            <div className="text-[#475569] text-[10px] font-bold uppercase">Ventilators</div>
            <div className="text-sm font-black text-[#0F172A] mt-0.5">
          ...
          <div className="bg-[#F8FAFC] p-3 rounded-xl border border-[#E2E8F0]">
            <div className="text-[#475569] text-[10px] font-bold uppercase">ER Load</div>
            <div className="text-sm font-black text-[#0F172A] mt-0.5">
              {hospital.er_load_score}/5
            </div>
          </div>

          <div className="col-span-2 bg-[#F8FAFC] p-3 rounded-xl border border-[#E2E8F0]">
            <div className="text-[#475569] text-[10px] font-bold uppercase">Specialists On Call</div>
            <div className="text-xs text-[#0F172A] mt-1 flex flex-wrap gap-1 font-sans">
              {hospital.specialists_on_call.map((s) => (
                <span key={s} className="px-2 py-0.5 bg-[#E2E8F0] text-[#0F172A] rounded-md text-[11px] font-bold">
                  {s.replace('_', ' ')}
                </span>
              ))}
            </div>
          ...
          <div className="col-span-2 bg-[#F8FAFC] p-3 rounded-xl border border-[#E2E8F0]">
            <div className="text-[#475569] text-[10px] font-bold uppercase">Blood Inventory</div>
            <div className="text-xs text-[#0F172A] mt-1 flex flex-wrap gap-1.5">
              {Object.entries(hospital.blood_stock).map(([type, units]) => (
                <span key={type} className="text-[#475569] font-mono">
                  {type}: <b className="text-[#0F172A] font-bold">{units}</b>
                </span>
              ))}
            </div>
          </div>
