...
export const DistributionBoard: React.FC<DistributionBoardProps> = ({
  ...
  return (
    <div className="border border-[#E8E2D9] rounded-3xl bg-white p-6 sm:p-8 select-none space-y-6 shadow-sm relative overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-5 border-b border-[#E8E2D9]">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="font-mono text-xs font-bold text-[#354F52] bg-[#EFF6F3] px-3 py-0.5 rounded-full border border-[#52796F]/40 uppercase tracking-wider">
              Incident Response
            </span>
          ...
          <h2 className="text-2xl font-display font-black text-[#2D231C] tracking-tight">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={clsx(
              ...
              showComparison
                ? 'border-[#D8CFBF] bg-[#FEF3C7] text-[#B45309]'
                : 'border-[#E8E2D9] bg-white text-[#7D7067] hover:text-[#2D231C] hover:bg-[#FAF8F5]'
            )}
          >
      ...
      {showComparison ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
          ...
          <div className="p-6 rounded-2xl border-2 border-[#E11D48]/40 bg-[#FFE4E6]/25 space-y-4 shadow-xs">
            ...
            <p className="text-xs text-[#7D7067] font-medium leading-relaxed font-sans">
              Without capability matching, dispatch sends all 4 incoming casualties to the nearest single general hospital (<b className="text-[#2D231C]">CityCare General</b>), saturating its ER and exhausting ICU beds.
            </p>

            <div className="p-4 bg-white rounded-xl border border-[#E11D48]/30 space-y-2.5 shadow-xs">
              <div className="flex justify-between text-xs font-mono font-bold">
                <span className="text-[#7D7067]">CityCare ER Load:</span>
                <span className="text-[#E11D48]">5 / 5 (100% SATURATED)</span>
              </div>
              <div className="flex justify-between text-xs font-mono font-bold">
                <span className="text-[#7D7067]">ICU Beds Available:</span>
                <span className="text-[#E11D48]">0 / 1 (DEPLETED)</span>
              </div>
              <div className="flex justify-between text-xs font-mono font-bold">
                <span className="text-[#7D7067]">Unplaceable Patients:</span>
          ...
          <div className="p-6 rounded-2xl border-2 border-[#52796F]/40 bg-[#EFF6F3]/25 space-y-4 shadow-xs">
            <div className="flex items-center justify-between border-b border-[#52796F]/40 pb-3">
              <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-mono bg-[#EFF6F3] text-[#354F52] px-2.5 py-0.5 rounded-full font-bold">
                OPTIMIZED
              </span>
            </div>
            ...
            <div className="space-y-2.5 font-mono text-xs">
              <div className="p-3 bg-white rounded-xl border border-[#52796F]/40 flex justify-between items-center shadow-xs">
                <span className="text-[#2D231C] font-bold">Trauma Patient → Apex Centre:</span>
                <span className="text-[#52796F] font-bold">Surgeon ready, O- blood reserved</span>
              </div>
              <div className="p-3 bg-white rounded-xl border border-[#52796F]/40 flex justify-between items-center shadow-xs">
                <span className="text-[#2D231C] font-bold">Cardiac Patient → Metro Heart:</span>
                <span className="text-[#52796F] font-bold">Cath lab prep, cardiologist on shift</span>
              </div>
              <div className="p-3 bg-white rounded-xl border border-[#52796F]/40 flex justify-between items-center shadow-xs">
                <span className="text-[#2D231C] font-bold">Pediatric Patient → Lifeline:</span>
                <span className="text-[#52796F] font-bold">Pediatrician & PICU reserved</span>
              </div>
            </div>

            <div className="text-[11px] font-mono text-[#52796F] font-bold">
        ...
        <div className="space-y-6">
          ...
          <div className="relative p-6 sm:p-8 rounded-2xl bg-[#FAF8F5] border border-[#E8E2D9] min-h-[300px] flex flex-col justify-between overflow-hidden shadow-inner">
            {/* Stage Indicator */}
            <div className="flex justify-between items-center text-xs font-mono text-[#7D7067] mb-4">
              <span className="font-bold">Dynamic Routing Engine:</span>
              <span className="text-[#EA580C] font-bold uppercase tracking-wider">
                ...
                return (
                  <div
                    ...
                    className={clsx(
                      ...
                      assignment
                        ? 'border-[#EA580C] shadow-md shadow-[#EA580C]/10'
                        : 'border-[#E8E2D9] shadow-xs'
                    )}
                    style={{
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between text-[11px] font-mono mb-2">
                        <span className="text-[#2D231C] font-extrabold">P-{idx + 1}</span>
                        <span className={clsx(
                      ...
                      <div className="text-xs text-[#7D7067] font-mono font-bold mb-1">
                        Age {c.patient_basic_info.age} • <span className="capitalize">{c.patient_basic_info.sex}</span>
                      </div>
                      <div className="text-[11px] text-[#7D7067] line-clamp-1 mb-3 font-medium">
                        {c.vitals_summary}
                      </div>
                    </div>

                    {assignment ? (
                      <div className="pt-3 border-t border-[#E8E2D9] animate-fade-in space-y-1">
                        <div className="text-[10px] font-mono uppercase text-[#EA580C] flex items-center gap-1 font-bold">
                          <ArrowRight className="w-3 h-3" />
                          <span>Routed To:</span>
                        </div>
                        <div className="text-xs font-display font-bold text-[#2D231C] truncate">
                          {assignment.assigned_hospital_name}
                        </div>
                        <div className="text-[10px] text-[#52796F] font-mono font-bold">
                          {assignment.reason}
                        </div>
                      </div>
                    ) : (
                      <div className="pt-3 border-t border-[#E8E2D9] text-[10px] font-mono text-[#7D7067] font-semibold">
                        Pending assignment
                      </div>
                    )}
                  </div>
            ...
            {(animStage === 'converge' || animStage === 'hold') && (
              <div className="absolute inset-0 flex items-center justify-center bg-white/90 backdrop-blur-md animate-fade-in z-20">
                <div className="flex flex-col items-center">
                  <div className="w-16 h-16 rounded-full border-2 border-[#EA580C] animate-ping flex items-center justify-center mb-3">
                    <Split className="w-7 h-7 text-[#EA580C]" />
                  </div>
                  <span className="font-mono text-xs text-[#2D231C] font-bold">
          ...
          {assignments.length === 0 && onDistributeAll && (
            <div className="flex justify-end pt-2">
              <button
                ...
                className="px-8 py-3.5 bg-[#EA580C] hover:bg-[#C2410C] text-white font-display font-black rounded-2xl text-sm transition-all shadow-md shadow-[#EA580C]/25 active:scale-95 disabled:opacity-50 select-none btn-tactile flex items-center gap-2"
              >
